...
function cubiesInLayer(moveBase: string, cubies: THREE.Mesh[]): THREE.Mesh[] {
  return cubies.filter((m) => {
    const p = getWorldPos(m)
    switch (moveBase) {
      case 'U':
        return p.y > EPS
      case 'D':
...
function scrambleCube(scrambleLength = 22) {
...
async function runNextMove() {
  ...
  if (!scene || cubies.length !== 8) return
